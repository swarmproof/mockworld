...
> **Status:** original research, now implemented and released as `mockworld-mcp`.
> "✅ RESOLVED" notes reflect decisions at the time; see `../ROADMAP.md` and
...
|---|----------|-------|-----------|
| Q1 | ✅ **RESOLVED (2026-07-13).** trace-format is an **OTel GenAI semantic-conventions profile** (not bespoke). mockworld emits `gen_ai.*` + `swarmproof.*`; target-side span = `span.kind=SERVER` parented to stampede's `execute_tool` CLIENT span, joined on `gen_ai.tool.call.id`; faults as `swarmproof.fault.kind` (shipped name; the drafted `type/injected/source` triple was consolidated); propagation via `traceparent` (HTTP headers / MCP `_meta`). See ARCHITECTURE §7.3. | stampede ✔ | ARCHITECTURE §7, observability |
